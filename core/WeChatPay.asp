...
'=   联系邮箱：mqycn@126.com                                    =
'================================================================
'=   使用说明：http://www.miaoqiyuan.cn/p/asp-wechat-pay        =
...
class WeChatPay
    ...
    public sub Class_Initialize()
        ...
        AppSecret   = "App密码"
        MchID       = "商户ID"
        MchKey      = "商户API密钥" '在 微信支付后台 \ 帐户中心 \ API安全，设置 API密钥 中设置
    ...
    public function Pay(byval out_trade_no, byval subject, byval body, byval total_fee)
        ...
        remote_addr = Request.Servervariables("HTTP_X_FORWARDED_FOR")
        if trim(remote_addr) <> "" then
            if Not isNumeric(replace(remote_addr, ".", "")) then remote_addr = ""
        end if
        if remote_addr = "" then remote_addr = Request.Servervariables("REMOTE_ADDR")
        
        OrderArr = Array("body=" & subject, "total_fee=" & total_fee, "out_trade_no=" & out_trade_no, "notify_url=" & notifyUrl, "spbill_create_ip=" & remote_addr, "trade_type=" & trade_type, "appid=" & AppID, "mch_id=" & MchID, "nonce_str=" & nonce_str)
    ...
    public function GetNotify()
        if Request.TotalBytes = 0 then
            set GetNotify = CreateResult(false, "Bad Request")
            exit function
        end if
        '必须通过二进制获取，这是一个大坑
        '二进制获取，只能获取一次
        ...
        if GetParaValue(resultPara, "return_code") <> "SUCCESS" and GetParaValue(resultPara, "return_msg") <> "OK" then
            ...
            resultPara = sortPara(resultPara)
            if Sign(resultPara) = GetParaValue(resultPara, "sign") then
                set GetNotify = CreateResult(true, "ok")
                call GetNotify.add("trade_no", GetParaValue(resultPara, "transaction_id"))
                call GetNotify.add("total_fee", cSng(GetParaValue(resultPara, "total_fee")))
                call GetNotify.add("out_trade_no", GetParaValue(resultPara, "out_trade_no"))
                call GetNotify.add("fee_type", GetParaValue(resultPara, "fee_type"))
                call GetNotify.add("is_subscribe", GetParaValue(resultPara, "is_subscribe"))
                call GetNotify.add("mch_id", GetParaValue(resultPara, "mch_id"))
                call GetNotify.add("openid", GetParaValue(resultPara, "openid"))
                call GetNotify.add("time_end", GetParaValue(resultPara, "time_end"))
                call GetNotify.add("trade_type", GetParaValue(resultPara, "trade_type"))
            else
    ...
    '统一返回结果，和之前的 支付宝接口统一
    private function CreateResult(byval status, byval message)
        set CreateResult = Server.CreateObject("Scripting.Dictionary")
        call CreateResult.add("status", status)
        call CreateResult.add("message", message)
    end function
    ...
    private function XMLRequest(byval sUrl, byval xmlBody)
        ...
        xmlhttp.SetClientCertificate("LOCAL_MACHINE\My\" & CertName)
